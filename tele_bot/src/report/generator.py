...
from ..data.tickers import ALL_FX_PAIRS, G10_PAIRS, EM_ASIA_PAIRS, CROSS_ASSET, ALL_PCA_ETFS
...
from ..analysis.pca_etf import build_pca_etf_report
from ..analysis.pca_fx import build_pca_fx_report
...
class ReportGenerator:
    ...
    def _load_pca_etf_data(self) -> dict[str, pd.DataFrame]:
        result = {}
        for symbol in ALL_PCA_ETFS:
            df = self._cache.get_cross_asset(symbol)
            if df is not None and not df.empty:
                result[symbol] = df
            else:
                logger.warning("No data for PCA ETF %s", symbol)
        return result
    ...
    def latest_daily_date(self) -> str:
        """Return the latest daily bar date as a string (from EURUSD cache)."""
        try:
            last = self._cache.daily_last_date("EURUSD")
            return str(last) if last else ""
        except Exception:
            return ""

    def latest_hourly_date(self) -> str:
        """Return the latest hourly bar date as a string (from EURUSD cache)."""
        try:
            last = self._cache.hourly_last_date("EURUSD")
            return str(last) if last else ""
        except Exception:
            return ""
    ...
    def refresh_data(self, include_hourly: bool = True, force: bool = False) -> None:
        """Incremental data refresh from Polygon."""
        self._refresher.refresh_daily_fx(force=force)
        self._refresher.refresh_cross_asset(force=force)
        self._refresher.refresh_pca_etfs(force=force)
        if include_hourly:
            self._refresher.refresh_hourly_fx(force=force)
    ...
    def generate_pca_etf(self, window: int = 120, n_components: int = 5) -> dict | None:
        etf_data = self._load_pca_etf_data()
        return build_pca_etf_report(etf_data, window=window, n_components=n_components)

    def generate_pca_fx(self, window: int = 120, n_components: int = 3) -> dict | None:
        fx_data = self._load_g10_daily()
        return build_pca_fx_report(fx_data, window=window, n_components=n_components)
    ...
    def generate_morning_brief(self) -> dict:
        ...
        return {
            ...
            "pca_etf": self.generate_pca_etf(),
            "pca_fx": self.generate_pca_fx(),
        }
