...
def format_pca_etf(report: dict | None, timestamp: str | None = None) -> list[str]:
    """Format PCA ETF factor decomposition for Telegram."""
    header = _header("PCA - ETF Factor Decomposition", timestamp)

    if report is None:
        return [header + "\nInsufficient ETF data for PCA."]

    regime = report.get("regime", "Unknown")
    eff_dim = report.get("effective_dim", 0)
    window = report.get("window", 0)
    n_assets = report.get("n_assets", 0)

    summary = (
        f"Regime: <b>{regime}</b> | Eff.Dim: {eff_dim:.1f} | "
        f"Window: {window}d | Assets: {n_assets}\n"
    )

    # Variance explained table
    var_exp = report.get("variance_explained", [])
    cum_var = report.get("cumulative_variance", [])
    eigenvalues = report.get("eigenvalues", [])
    ...
    var_table = tabulate(
        var_rows,
        headers=["PC", "Eigenval", "Var%", "Cum%"],
        tablefmt="plain",
        stralign="right",
    )

    messages = [header + summary + "\n" + _pre(var_table)]

    # Top/bottom loadings per PC
    top_loadings = report.get("top_loadings_per_pc", {})
    for pc, data in top_loadings.items():
        top = data.get("top")
        bottom = data.get("bottom")
        rows = []
        if top is not None:
            for sym, val in top.items():
                rows.append([sym, f"{val:+.3f}", "+"])
        if bottom is not None:
            for sym, val in bottom.items():
                rows.append([sym, f"{val:+.3f}", "-"])
        ...
        msg = _pre(tbl)
        if messages and len(messages[-1]) + len(msg) < MAX_MSG_LEN:
            messages[-1] += "\n" + msg
        else:
            messages.append(msg)
...
def format_pca_fx(report: dict | None, timestamp: str | None = None) -> list[str]:
    """Format PCA FX factor analysis for Telegram."""
    header = _header("PCA - FX Factor Analysis", timestamp)

    if report is None:
        return [header + "\nInsufficient FX data for PCA."]
    ...
    summary = (
        f"Regime: <b>{regime}</b> | Eff.Dim: {eff_dim:.1f} | Window: {window}d\n"
        f"Factors: {label_str}\n"
    )
    ...
    var_rows = []
    for i in range(len(var_exp)):
        pc_label = labels.get(f"PC{i + 1}", f"PC{i + 1}")
        var_rows.append([
            f"PC{i + 1}",
            pc_label,
            f"{var_exp[i] * 100:.1f}%",
    ...
    if loadings is not None:
        load_rows = []
        for pair in loadings.index:
            row = [pair]
            for col in loadings.columns:
                row.append(f"{loadings.loc[pair, col]:+.3f}")
            load_rows.append(row)

        load_table = tabulate(
            load_rows,
            headers=["Pair"] + list(loadings.columns),
            tablefmt="plain",
            stralign="right",
        )
    else:
        load_table = "No loadings data."
    ...
    score_rows = []
    if pc_scores is not None and not pc_scores.empty:
        for pc in pc_scores.index:
            score = pc_scores[pc]
            z = pc_zscores[pc] if pc_zscores is not None and pc in pc_zscores.index else 0
            score_rows.append([pc, f"{score:+.3f}", f"{z:+.2f}"])

    score_table = tabulate(
        score_rows,
        headers=["PC", "Score", "Z-score"],
        tablefmt="plain",
        stralign="right",
    ) if score_rows else ""

    msg1 = header + summary + "\n" + _pre(var_table)
    msg2 = _pre(load_table)

    messages = [msg1]
    if len(msg1) + len(msg2) < MAX_MSG_LEN:
        messages[0] += "\n" + msg2
    else:
        messages.append(msg2)

    if score_table:
        score_msg = _pre(score_table)
        if len(messages[-1]) + len(score_msg) < MAX_MSG_LEN:
            messages[-1] += "\n" + score_msg
        else:
...
def format_full_report(report: dict) -> list[str]:
    ...
    # PCA ETF
    pca_etf = report.get("pca_etf")
    if pca_etf is not None:
        messages.extend(format_pca_etf(pca_etf, ts))

    # PCA FX
    pca_fx = report.get("pca_fx")
    if pca_fx is not None:
        messages.extend(format_pca_fx(pca_fx, ts))
